...
class AdminPageFramework_FieldType_select extends AdminPageFramework_FieldType {
    ...
    protected function getStyles() {
        return ".dgenny-field-select .dgenny-input-label-container {vertical-align: top; }.dgenny-field-select .dgenny-input-label-container {padding-right: 1em;}";
    }
    protected function getField($aField) {
        ...
        return $aField['before_label'] . "<div " . $this->getLabelContainerAttributes($aField, 'dgenny-input-label-container dgenny-select-label') . ">" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . $_oSelectInput->get($aField['label']) . $aField['after_input'] . "<div class='repeatable-field-buttons'></div>" . "</label>" . "</div>" . $aField['after_label'];
    }
    }
    class AdminPageFramework_FieldType_size extends AdminPageFramework_FieldType_select {
        ...
        protected function getStyles() {
            return ".dgenny-field-size input {text-align: right;}.dgenny-field-size select.size-field-select {vertical-align: 0px; }.dgenny-field-size label {width: auto; } .form-table td fieldset .dgenny-field-size label {display: inline-block;}";
        }
        protected function getField($aField) {
        ...
        private function _getNumberInputPart(array $aField, array $aBaseAttributes, $isKey, $bMultiLabels) {
            ...
            $_sLabel = $this->getElementByLabel($aField['label'], $isKey, $aField['label']);
            return "<label " . $this->getAttributes($_aSizeLabelAttributes) . ">" . $this->getElement($aField, $bMultiLabels ? array('before_label', $isKey, 'size') : array('before_label', 'size')) . ($aField['label'] && !$aField['repeatable'] ? "<span " . $this->getLabelContainerAttributes($aField, 'dgenny-input-label-string') . ">" . $_sLabel . "</span>" : "") . "<input " . $this->getAttributes($_aSizeAttributes) . " />" . $this->getElement($aField, $bMultiLabels ? array('after_input', $isKey, 'size') : array('after_input', 'size')) . "</label>";
        }
