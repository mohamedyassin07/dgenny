...
class AdminPageFramework_FieldType_system extends AdminPageFramework_FieldType {
    ...
    protected function getStyles() {
        return ".dgenny-field-system {width: 100%;}.dgenny-field-system .dgenny-input-label-container {width: 100%;}.dgenny-field-system textarea {background-color: #f9f9f9; width: 97%; outline: 0; font-family: Consolas, Monaco, monospace;white-space: pre;word-wrap: normal;overflow-x: scroll;}";
    }
    protected function getField($aField) {
        $_aInputAttributes = $aField['attributes'];
        $_aInputAttributes['class'].= ' system';
        unset($_aInputAttributes['value']);
        return $aField['before_label'] . "<div class='dgenny-input-label-container'>" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . ($aField['label'] && !$aField['repeatable'] ? "<span " . $this->getLabelContainerAttributes($aField, 'dgenny-input-label-string') . ">" . $aField['label'] . "</span>" : "") . "<textarea " . $this->getAttributes($_aInputAttributes) . " >" . esc_textarea($this->___getSystemInformation($aField['value'], $aField['data'], $aField['print_type'])) . "</textarea>" . $aField['after_input'] . "</label>" . "</div>" . $aField['after_label'];
    }
    ...
    private function ___getClientInfo($bGenerateInfo = true) {
        ...
        return empty($_aBrowser) ? __('No browser information found.', 'dgenny') : $_aBrowser;
    }
    ...
    private function ___getSiteInfo() {
        global $wpdb;
        return array(__('Version', 'dgenny') => $GLOBALS['wp_version'], __('Language', 'dgenny') => $this->getSiteLanguage(), __('Memory Limit', 'dgenny') => $this->getReadableBytes($this->getNumberOfReadableSize(WP_MEMORY_LIMIT)), __('Multi-site', 'dgenny') => $this->getAOrB(is_multisite(), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Permalink Structure', 'dgenny') => get_option('permalink_structure'), __('Active Theme', 'dgenny') => $this->___getActiveThemeName(), __('Registered Post Statuses', 'dgenny') => implode(', ', get_post_stati()), 'WP_DEBUG' => $this->getAOrB($this->isDebugMode(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), 'WP_DEBUG_LOG' => $this->getAOrB($this->isDebugLogEnabled(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), 'WP_DEBUG_DISPLAY' => $this->getAOrB($this->isDebugDisplayEnabled(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), __('Table Prefix', 'dgenny') => $wpdb->prefix, __('Table Prefix Length', 'dgenny') => strlen($wpdb->prefix), __('Table Prefix Status', 'dgenny') => $this->getAOrB(strlen($wpdb->prefix) > 16, $this->oMsg->get('too_long'), $this->oMsg->get('acceptable')), 'wp_remote_post()' => $this->___getWPRemotePostStatus(), 'wp_remote_get()' => $this->___getWPRemoteGetStatus(), __('Multibite String Extension', 'dgenny') => $this->getAOrB(function_exists('mb_detect_encoding'), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), __('WP_CONTENT_DIR Writeable', 'dgenny') => $this->getAOrB(is_writable(WP_CONTENT_DIR), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Active Plugins', 'dgenny') => PHP_EOL . $this->___getActivePlugins(), __('Network Active Plugins', 'dgenny') => PHP_EOL . $this->___getNetworkActivePlugins(), __('Constants', 'dgenny') => $this->___getDefinedConstants('user'),);
    }
    ...
    private function _getPHPInfo($bGenerateInfo = true) {
        ...
        self::$_aPHPInfo = array(__('Version', 'dgenny') => phpversion(), __('Safe Mode', 'dgenny') => $this->getAOrB(@ini_get('safe_mode'), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Memory Limit', 'dgenny') => @ini_get('memory_limit'), __('Upload Max Size', 'dgenny') => @ini_get('upload_max_filesize'), __('Post Max Size', 'dgenny') => @ini_get('post_max_size'), __('Upload Max File Size', 'dgenny') => @ini_get('upload_max_filesize'), __('Max Execution Time', 'dgenny') => @ini_get('max_execution_time'), __('Max Input Vars', 'dgenny') => @ini_get('max_input_vars'), __('Argument Separator', 'dgenny') => @ini_get('arg_separator.output'), __('Allow URL File Open', 'dgenny') => $this->getAOrB(@ini_get('allow_url_fopen'), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Display Errors', 'dgenny') => $this->getAOrB(@ini_get('display_errors'), $this->oMsg->get('on'), $this->oMsg->get('off')), __('Log Errors', 'dgenny') => $this->getAOrB(@ini_get('log_errors'), $this->oMsg->get('on'), $this->oMsg->get('off')), __('Error log location', 'dgenny') => @ini_get('error_log'), __('Error Reporting Level', 'dgenny') => $_oErrorReporting->getErrorLevel(), __('FSOCKOPEN', 'dgenny') => $this->getAOrB(function_exists('fsockopen'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), __('cURL', 'dgenny') => $this->getAOrB(function_exists('curl_init'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), __('SOAP', 'dgenny') => $this->getAOrB(class_exists('SoapClient'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), __('SUHOSIN', 'dgenny') => $this->getAOrB(extension_loaded('suhosin'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), 'ini_set()' => $this->getAOrB(function_exists('ini_set'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')),) + $this->getPHPInfo() + array(__('Constants', 'dgenny') => $this->___getDefinedConstants(null, 'user'));
        return self::$_aPHPInfo;
    }
    private function ___getWebServerInfo($bGenerateInfo = true) {
        return $bGenerateInfo ? array(__('Web Server', 'dgenny') => $_SERVER['SERVER_SOFTWARE'], 'SSL' => $this->getAOrB(is_ssl(), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Session', 'dgenny') => $this->getAOrB(isset($_SESSION), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), __('Session Name', 'dgenny') => esc_html(@ini_get('session.name')), __('Session Cookie Path', 'dgenny') => esc_html(@ini_get('session.cookie_path')), __('Session Save Path', 'dgenny') => esc_html(@ini_get('session.save_path')), __('Session Use Cookies', 'dgenny') => $this->getAOrB(@ini_get('session.use_cookies'), $this->oMsg->get('on'), $this->oMsg->get('off')), __('Session Use Only Cookies', 'dgenny') => $this->getAOrB(@ini_get('session.use_only_cookies'), $this->oMsg->get('on'), $this->oMsg->get('off')),) + $_SERVER : '';
    }
    }
