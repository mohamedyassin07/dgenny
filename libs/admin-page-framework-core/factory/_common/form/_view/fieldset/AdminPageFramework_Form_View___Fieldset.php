...
jQuery( document ).ready( function() {
    var _oButtonPlaceHolders = jQuery( '#{$sFieldsContainerID} > .dgenny-field.without-child-fields .repeatable-field-buttons' );
    ...
    /* Otherwise, insert the button element at the beginning of the field tag */
         ...
         */
        if ( ! jQuery( '#{$sFieldsContainerID} .dgenny-repeatable-field-buttons' ).length ) { 
            jQuery( '#{$sFieldsContainerID} > .dgenny-field.without-nested-fields' ).prepend( $_sSmallButtons );
        }
        /**
         * Support for nested fields.
         * For nested fields, add the buttons to the fields tag.
         */
        jQuery( '#{$sFieldsContainerID} > .dgenny-field.with-nested-fields' ).prepend( $_sNestedFieldsButtons );
        
        /**
         * Support for inline mixed fields.
         */
        // jQuery( '#{$sFieldsContainerID} > .dgenny-field.with-mixed-fields' ).prepend( $_sNestedFieldsButtons );
...
JAVASCRIPTS;
    ...
    private function ___getContainerAttributes($aArguments) {
        $_aContainerAttributes = array('class' => $this->getClassAttribute('dgenny-repeatable-field-buttons', !empty($aArguments['disabled']) ? 'disabled' : ''),);
...
JAVASCRIPTS;
        return "<script type='text/javascript' class='dgenny-sortable-field-enabler-script'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
    }
    }
    class AdminPageFramework_Form_View___Fieldset extends AdminPageFramework_Form_View___Fieldset_Base {
        ...
        private function _getChildFieldTitle($aFieldset) {
            ...
            $_oFieldTitle = new AdminPageFramework_Form_View___FieldTitle($aFieldset, array('dgenny-child-field-title'), $this->aOptions, $this->aErrors, $this->aFieldTypeDefinitions, $this->aCallbacks, $this->oMsg);
            return $_oFieldTitle->get();
        }
        ...
        private function _getExtras($aField, $iFieldsCount) {
            $_aOutput = array();
            $_oFieldDescription = new AdminPageFramework_Form_View___Description($aField['description'], 'dgenny-fields-description');
