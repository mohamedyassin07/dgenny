...
(function ( $ ) {
    ...
    $.fn.updateAdminPageFrameworkRepeatableFields = function( aSettings ) {
        ...
        var _oRepeatableButtons = $( nodeThis ).find( '.dgenny-repeatable-field-buttons' )
            .filter( function() {                                       
                return $( this ).closest( '.dgenny-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
            });       
        ...
        var _sFieldID           = _oRepeatableAddButtons.first().closest( '.dgenny-field' ).attr( 'id' );
        var _nCurrentFieldCount = $( '#' + _sFieldsContainerID ).find( '.dgenny-field' )
            .filter( function() {                 
                return $( this ).closest( '.dgenny-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
            })        
            .length;
    ...
    $.fn.addAdminPageFrameworkRepeatableField = function( sFieldContainerID ) {
        ...
        if ( 'undefined' === typeof sFieldContainerID ) {
            var sFieldContainerID = $( this ).closest( '.dgenny-field' ).attr( 'id' );
        }

        var nodeFieldContainer  = $( '#' + sFieldContainerID );
        var nodeNewField        = nodeFieldContainer.clone(); // clone without bind events.
        var nodeFieldsContainer = nodeFieldContainer.closest( '.dgenny-fields' );
        ...
        if ( ! _aOptions ) {     
            var _nodeButtonContainer = nodeFieldContainer.find( '.dgenny-repeatable-field-buttons' )
                .filter( function() {                 
                    return $( this ).closest( '.dgenny-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                })
                .first();
        ...
        var _oInnerFields       = nodeFieldsContainer.find( '.dgenny-field' ) 
                                    .filter( function() {                 
                                        return $( this ).closest( '.dgenny-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                                    });
        var _oRepeatableButtons = nodeFieldContainer.find( '.dgenny-repeatable-field-buttons' )
                                    .filter( function() {                 
                                        return $( this ).closest( '.dgenny-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                                    }); 
        ...
        $( nodeNewField ).find( '.dgenny-field' ).addBack().trigger( 
            'dgenny_repeated_field', 
            [ 
                0, // call type, 0 : repeatable fields, 1: repeatable sections
                jQuery( nodeNewField ).closest( '.dgenny-fields' )    // model container
            ]
        );
        ...
        var nodeRemoveButtons = nodeFieldsContainer
            ...
            .filter( function() {                      
                return $( this ).closest( '.dgenny-fields' ).attr( 'id' ) === _sFieldsContainerID;                
            })
        ...
        nodeFieldsContainer.children( '.dgenny-field' ).children( '.delimiter' ).show().last().hide();
        ...
        var _incrementFieldAttributes = function( oElement, oFieldsContainer ) {
            ...
            oElement.find( '.dgenny-field,.dgenny-fields,.dgenny-fieldset' ).incrementAttributes(
    ...
    $.fn.removeAdminPageFrameworkRepeatableField = function() {
        ...
        var nodeFieldContainer  = $( this ).closest( '.dgenny-field' );
        var nodeFieldsContainer = $( this ).closest( '.dgenny-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );
        var _aOptions = nodeFieldsContainer.data( 'repeatable' );
