...
( function( $ ) {
     ...
     * @since    3.7.0
     */
    jQuery( document ).on( 'dgenny_saved_widget', function( event, oWidget ){
    ...
    $.fn.initializeAdminPageFrameworkCollapsibleSections = function() {
        ...
        jQuery( this ).find( '.dgenny-collapsible-sections-title[data-is_collapsed=\"0\"]' )
            .next( '.dgenny-collapsible-sections-content' )
            .slideDown( 'fast' );
        jQuery( this ).find( '.dgenny-collapsible-section-title[data-is_collapsed=\"0\"]' )
            .closest( '.dgenny-section-table' )
            .find( 'tbody' )
            .slideDown( 'fast' );
            
        // Hide collapsible sections of 'section' containers as they are somehow do not get collapsed by default.
        jQuery( this ).find( '.dgenny-collapsible-section-title[data-is_collapsed=\"1\"]' )
            .closest( '.dgenny-section-table' )
            .find( 'tbody' )
            .hide();
        
        // Bind the click event to the title element.
        jQuery( this ).find( '.dgenny-collapsible-sections-title, .dgenny-collapsible-section-title' )
            .enableAdminPageFrameworkCollapsibleButton();
        
        // Insert the toggle all button.
        jQuery( this ).find( '.dgenny-collapsible-title[data-toggle_all_button!=\"0\"]' ).each( function(){
            
            var _oThis        = jQuery( this ); // to access from inside the below each() method.
            var _bForSections = jQuery( this ).hasClass( 'dgenny-collapsible-sections-title' );   // or for the 'section' container.
            ...
            jQuery.each( _aPositions, function( iIndex, _sPosition ) {
                ...
                var _sLeftOrRight = -1 !== jQuery.inArray( _sPosition, [ 'top-right', 'bottom-right', '0' ] )   // if found
                    ? 'right'   // default
                    : 'left';            
                _oButton.find( '.dgenny-collapsible-toggle-all-button' ).css( 'float', _sLeftOrRight );
                ...
                if ( _bForSections ) {
                    var _oTargetElement = 'before' === _sTopOrBottom
                        ? _oThis
                        : _oThis.next( '.dgenny-collapsible-content' );
                        _oTargetElement[ _sTopOrBottom ]( _oButton );
                } else {    // for 'section' containers
                    _oThis.closest( '.dgenny-section' )[ _sTopOrBottom ]( _oButton );
                }                
                ...
                _oButton.on( 'click', function(){                  
                    ...
                    var _oButtons = _bForSections
                        ? jQuery( this ).closest( '.dgenny-sectionset' ).siblings().andSelf().find( '> .dgenny-collapsible-toggle-all-button-container' )
                        : jQuery( this ).siblings( '.dgenny-collapsible-toggle-all-button-container' ).andSelf();
                    _oButtons.toggleClass( 'flipped' );
                    if ( _bForSections ) {
                        _oButton.parent().parent().children().children( '* > .dgenny-collapsible-title' ).each( function() {
                            jQuery( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    } else {
                        _oButton.closest( '.dgenny-sections' ).children( '.dgenny-section' ).children( '.dgenny-section-table' ).children( 'caption' ).children( '.dgenny-collapsible-title' ).each( function() {
                            jQuery( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    }
    ...
    $.fn.enableAdminPageFrameworkCollapsibleButton = function() {
        ...
        function _isFieldElement( nodeTarget ) {
            ...
            if ( jQuery( nodeTarget ).hasClass( 'dgenny-collapsible-button' ) ) {
                return false;
            }
        ...
        jQuery( this ).on( 'click', function( event, sContext ){
            ...
            var _sContainerType = jQuery( this ).hasClass( 'dgenny-collapsible-sections-title' )
                ? 'sections'
                : 'section';
            var _oTargetContent = 'sections' === _sContainerType
                ? jQuery( this ).next( '.dgenny-collapsible-content' ).first()
            ...
            if ( 'expand' === _sAction && _oThis.data( 'collapse_others_on_expand' ) ) {
                _oThis.parent().parent().children().children( '* > .dgenny-collapsible-content' ).not( _oTargetContent ).slideUp( 'fast', function() {
                    jQuery( this ).prev( '.dgenny-collapsible-title' ).addClass( 'collapsed' );
                });
            }
...
JAVASCRIPTS;
    ...
    static private function _getToggleAllButtonHTML($oMsg) {
        ...
        return "<div class='dgenny-collapsible-toggle-all-button-container'>" . "<span class='dgenny-collapsible-toggle-all-button button " . $_sDashIconSort . "'" . " title='" . esc_attr($_sLabelToggleAllSections) . "'>" . $_sText . "</span>" . "</div>";
    }
    }
