...
( function( $ ) {
    ...
    $.fn.updateAdminPageFrameworkRepeatableSections = function( aSettings ) {
        ...
        var _oThis                = this;
        var _sSectionsContainerID = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.dgenny-sections' ).attr( 'id' );
        ...
        var _sSectionID           = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.dgenny-section' ).attr( 'id' );
        var _nCurrentSectionCount = jQuery( '#' + _sSectionsContainerID ).find( '.dgenny-section' ).length;
    ...
    $.fn.addAdminPageFrameworkRepeatableSection = function( sSectionContainerID ) {
        ...
        if ( 'undefined' === typeof sSectionContainerID ) {
            var sSectionContainerID = $( this ).closest( '.dgenny-section' ).attr( 'id' );
        }
        var nodeSectionContainer    = $( '#' + sSectionContainerID );
        var nodeNewSection          = nodeSectionContainer.clone(); // clone without bind events.
        var nodeSectionsContainer   = nodeSectionContainer.closest( '.dgenny-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );        
        var nodeTabsContainer       = $( this ).closest( '.dgenny-section-tabs-contents' )
            .children( '.dgenny-section-tabs' )
            .first();
        ...
        if ( _sMaxNumberOfSections != 0 && nodeSectionsContainer.find( '.dgenny-section' ).length >= _sMaxNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.dgenny-repeatable-section-buttons' ).last();
        ...
        var _sSectionTabSlug = nodeNewSection.find( '.dgenny-section-caption' ).first().attr( 'data-section_tab' );
        if ( ! _sSectionTabSlug || _sSectionTabSlug === '_default' ) {
            nodeNewSection.find( '.dgenny-section-title' ).not( '.dgenny-collapsible-section-title' ).hide();
        }
        ...
        if( 'function' === typeof nodeNewSection.enableAdminPageFrameworkCollapsibleButton ){ 
            nodeNewSection.find( '.dgenny-collapsible-sections-title, .dgenny-collapsible-section-title' ).enableAdminPageFrameworkCollapsibleButton();
        }
        ...
        $( nodeNewSection ).find( '.dgenny-field' ).each( function( iFieldIndex ) {
            ...
            // 3.8.8 
            $( this ).trigger( 
                'dgenny_repeated_field', 
                [
                    1, // call type, 0: repeatable fields, 1: repeatable sections, 
                    jQuery( nodeNewSection ).closest( '.dgenny-sections' )    // model container
                ]
            );              
        ...
        nodeNewSection.find( '.dgenny-fields.sortable' ).each( function() {
        ...
        if ( nodeTabsContainer.length > 0 && ! nodeSectionContainer.hasClass( 'is_subsection_collapsible' ) ) {
            ...
            nodeTabsContainer.closest( '.dgenny-section-tabs-contents' ).createTabs( 'refresh' );
        ...
        var _incrementAttributes = function( oElement, iSectionsCount, oSectionsContainer ) {
            ...
            $( oElement ).find( 'tr.dgenny-fieldrow, .dgenny-fieldset, .dgenny-fields, .dgenny-field, table.form-table, input,textarea,select,option' )
                .incrementAttribute( 
                    'id', 
                    iSectionsCount,
                    _sSectionIDModel
            ...
            $( oElement ).find( '.dgenny-fields' ).incrementAttribute( 
                'data-field_tag_id_model',
                iSectionsCount,
                _sSectionIDModel
            );
            $( oElement ).find( '.dgenny-fields' ).incrementAttributes( 
        ...
        // @todo this may be able to be removed
            $( oElement ).find( '.dgenny-fieldset' ).incrementAttribute( 
                'data-field_id',
                iSectionsCount,
                _sSectionIDModel
            );
    ...
    $.fn.removeAdminPageFrameworkRepeatableSection = function() {
        ...
        var nodeSectionContainer    = $( this ).closest( '.dgenny-section' );
        var sSectionContainerID     = nodeSectionContainer.attr( 'id' );
        var nodeSectionsContainer   = $( this ).closest( '.dgenny-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );        
        var nodeTabsContainer       = $( this ).closest( '.dgenny-section-tabs-contents' )
            .children( '.dgenny-section-tabs' )
            .first();
        var nodeTabs                = nodeTabsContainer.children( '.dgenny-section-tab' );
        ...
        if ( _sMinNumberOfSections != 0 && nodeSectionsContainer.find( '.dgenny-section' ).length <= _sMinNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.dgenny-repeatable-section-buttons' ).last();
        ...
        nodeSectionContainer.fadeOut( _iFadeout, function() { 
            ...
            if ( 1 === _nodeRemoveButtons.length ) {
                ...
                var _sSectionTabSlug = nodeSectionsContainer.find( '.dgenny-section-caption' ).first().attr( 'data-section_tab' );
                if ( ! _sSectionTabSlug || '_default' === _sSectionTabSlug ) {
                    nodeSectionsContainer.find( '.dgenny-section-title' ).first().show();
                }                
            }            
...
var _iSectionIndex = _iIterationIndex;
            ...
            // Call the registered callback functions.
            // @deprecated  3.6.0
            // $( this ).find( '.dgenny-field' ).each( function( iFieldIndex ) {    
...
jQuery( document ).ready( function() {
    ...
    // Adds the buttons
    jQuery( '#{$sContainerTagID} .dgenny-section-caption' ).each( function(){
        
        jQuery( this ).show();
        
        var _oButtons = jQuery( $_sButtonsHTML );
        if ( jQuery( this ).children( '.dgenny-collapsible-section-title' ).children( 'fieldset' ).length > 0 ) {
            _oButtons.addClass( 'section_title_field_sibling' );
        }
        var _oCollapsibleSectionTitle = jQuery( this ).find( '.dgenny-collapsible-section-title' );
...
JAVASCRIPTS;
        return "<script type='text/javascript' class='dgenny-section-repeatable-script'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
    }
