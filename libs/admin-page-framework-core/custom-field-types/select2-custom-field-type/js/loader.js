(function($){
    ...
    function initializeSelect2( oNode ) {
        ...
        if ( _aOptions[ 'width' ] && 'auto' !== _aOptions[ 'width' ] ) {
            var _oFieldContainer = _oSelect2Target.closest( '.dgenny-field-select2' );
            _oFieldContainer.css( 'width', _aOptions[ 'width' ] );
            _oFieldContainer.children( '.dgenny-select-label' )
                .css( 'width', '100%' );
            _oFieldContainer.children( '.dgenny-select-label' )
                .children( 'label' )
                .children( '.dgenny-input-container' )
                .css( 'width', '100%' );
            _aOptions[ 'width' ] = '100%';
        }
        ...
        if ( _aOptions[ 'search_callback' ] ) {
             ...
             * Set initial values.
             */
            var _oInputForEncoded = _oSelect2Target.closest( '.dgenny-field' )
    ...
    function templateResultForIcon( state ) {
        var _oIconDefinition = $( state.element ).closest( '.dgenny-field' ).find( '.custom-icon-definition[data-value=' + state.id + ']' );
